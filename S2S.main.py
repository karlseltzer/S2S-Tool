...
####################################################################################################
### User Input
### Photochemical mechanism; options include CB6R3_AE7, CB6R5_AE7, CB7_AE7, CRACMMv1.0,
### CRACMMv2.0, GEOSChem14.6.3, SAPRC07TC_AE7, CB6R3_AE7_TRACER, CB6R4_CF2, CB7_CF2, 
...
if MECH_BASIS in ('PM-AE6', 'PM-CR1', 'PM-CR2', 'PM-GC'):
    OUTPUT = 'PM'
else:
    OUTPUT = 'VOC'
### GSPRO/GSCNV output file names
if RUN_TYPE in ('INTEGRATE'):
...
### QA check: ensure mech4import, mechPM, profiles, tbl_tox, etc are not empty following filters above
check_inputs.check_inputs(molwght,mech4import,mechPM,profiles,tbl_tox,OUTPUT)
...
check_inputs.check_species_mech4import(mech4import,species,MECH_BASIS,OUTPUT)
### QA check: all PM profile types are in the SV-POA list
check_inputs.check_profiles_volatility(profiles,poa_volatility,OUTPUT)
### QA check: all SV-POA entries sum to 1
check_inputs.check_volatility(poa_volatility,OUTPUT)
### QA check: Ensure MECH_BASIS is included in poa_mapping file
check_inputs.check_mech_in_mapping(poa_mapping,MECH_BASIS,OUTPUT)

### Generate GSCNV file:
if OUTPUT=='VOC':
    if RUN_TYPE in {'CRITERIA', 'INTEGRATE', 'NOINTEGRATE'}:
        print('NOTICE: Beginning generation of GSCNV file.')
        gscnv.gen_gscnv(profiles, species, species_props, tbl_tox, gscnv_append,
            MECH_BASIS, RUN_TYPE, TOLERANCE, CNV_OUT)
        gscnv.format_and_header(MECH_BASIS, RUN_TYPE, AQM, MW_FILE, TOX_FILE, CNV_OUT)
        print('NOTICE: Finished generating GSCNV file.')
    else:
        sys.exit('There is an issue with your RUN_TYPE entry. Only CRITERIA, INTEGRATE, and NOINTEGRATE are accepted.')
elif OUTPUT=='PM':
    print(f'NOTICE: Type of Output is {OUTPUT}, no GSCNV generated.')
else:
    sys.exit('There is an issue with your OUTPUT entry. Only VOC and PM are accepted.')
...
if OUTPUT=='VOC':
    ...
    if AQM=='CMAQ' or (AQM=='CAMX' and MECH_BASIS=='PM-AE6'):
        if RUN_TYPE=='CRITERIA' or RUN_TYPE=='INTEGRATE':
            print('NOTICE: Beginning generation of GSPRO file.')
            ### Create gspro file for the target MECH_BASIS
            gspro.gen_gspro_pm(profiles,species,mechPM,tbl_tox,poa_volatility,poa_mapping,camx_fcrs,oxygen_metals,MECH_BASIS,RUN_TYPE,AQM,TOX_IN,PRO_OUT)
            ### Formats and adds header to gspro file
            gspro.format_and_header(tbl_tox,TOX_IN,MECH_BASIS,RUN_TYPE,AQM,MW_FILE,FCRS_FILE,TOX_FILE,PRO_OUT)
            print('NOTICE: Finished generating GSPRO file.')
